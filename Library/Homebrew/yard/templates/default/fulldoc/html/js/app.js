...
function summaryToggle(localStorage) {
...
function constantSummaryToggle(localStorage) {
  initializeSummaryToggle(
    localStorage,
...
function registerNavPointerMove(resizer, minimumNavWidth, sessionStorage) {
  resizer.addEventListener(
    "pointermove",
    function (e) {
...
function applyStoredNavWidth(minimumNavWidth, sessionStorage) {
  if (sessionStorage.navWidth) {
    applyNavWidth(minimumNavWidth, parseInt(sessionStorage.navWidth, 10));
  }
}

function navResizer(sessionStorage) {
  ...
  registerNavPointerMove(resizer, minimumNavWidth, sessionStorage);
  applyStoredNavWidth(minimumNavWidth, sessionStorage);
}
...
window.__app = function () {
  ...
  $(document).ready(function () {
    ...
    searchFrameButtons();
    linkSummaries();
    summaryToggle(localStorage);
    constantSummaryToggle(localStorage);
    generateTOC();
    mainFocus();
    navigationChange();
  });
